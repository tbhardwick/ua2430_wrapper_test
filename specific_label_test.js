...
console.log("\n=============================================");
console.log("ARINC-429 Single Label Test");
...
try {
    ...
    console.log(`Configuring channel ${RCV_CHAN} for AUTO SPEED (receive)...`);
    err = ua2430.config429(CHCFG429_AUTOSPEED, RCV_CHAN, hCore);
    if (err < 0) {
    ...
    // Create the receive filter for the same label
    console.log('Creating receive filter...');
    const rxFilterAddr = ua2430.filterSet(MSGCRT429_DEFAULT, TEST_LABEL, SDIALL, RCV_CHAN, hCore);
    console.log(`Filter address: 0x${rxFilterAddr.toString(16)}`);
    ...
    const interval = setInterval(() => {
        ...
        console.log(`  RX Data: 0x${rxData.toString(16).padStart(8, '0')}`);
